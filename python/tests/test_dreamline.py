...
# Element size [mm].
a = 10.0;
geom_par = np.asmatrix([a]);

H = dl.dreamline(Ro,geom_par,s_par,delay,m_par,"stop")

alpha  = 5.0;                   # Absorbtion (dB/cm Hz).
m_par = np.asmatrix([v,cp,alpha]);
Hatt = dl.dreamline(Ro,geom_par,s_par,delay,m_par,"stop");
...
if 'DO_PLOTTING' in locals():
    ...
    plt.title('Line strip transducer')
    plt.xlabel('t [us]')
