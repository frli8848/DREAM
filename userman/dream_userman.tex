...
\subsection{Delay-and-sum Imaging  --- SAFT, TFM and RCA}
\label{sec:das}

The DREAM Toolbox contains two delay-and-sum based reconstruction methods:
the \emph{synthetic aperture focusing} (SAFT) method and the \emph{total focusing method}
...
%%
where $\text{Im}(x_{\tilde{n}},z_m)$ is the beamformed image.
...
\begin{verbatim}
 Y     - K x L Ultrasonic B-scan data matrix.
         where K is the A-scan length and L is the number of transducer positions.
 G     - An L x 3 transducer position grid matrix [x1 y1 z2; x2 y2 z2; ... xL yL zL];
 Ro    - A matrix of the form  [xo1 yo1 zo2; xo2 yo2 zo2; ... xoN yoN zoN];
         where N is the number of obervation (reconstruction) points.
 delay - A scalar (or vector) of starting point(s) of the A-scans in Y.
 cp    - Sound speed [m/s].
\end{verbatim}
...
\begin{verbatim}
 Y     - K x L^2 Ultrasonic FMC B-scan data matrix.
         where K is the A-scan length and L is the number of transducer positions.
 Gt     - An L_t x 3 transmit element position grid matrix
          Gt = [xt1 yt1 zt2; xt2 yt2 zt2; ... xtL_t ytL_t ztL_t];
          where L_t is the number of transmit element positions.
 Gr     - An L_r x 3 receive element position grid matrix
          Gr = [xr1 yr1 zr2; xr2 yr2 zr2; ... xrL_r yrL_r zrL_r];
