...
% $Revision: 882 $ $Date: 2015-05-11 08:14:59 +0200 (Mon, 11 May 2015) $ $LastChangedBy: frli8848 $
...
\author{Fredrik Lingvall\footnote{E-mail:\texttt{Fredrik.Lingvall@gmail.com}}}
...
\item \href{http://www.fftw.org}{FFTW} $\ge$ 3.0.1 (optional).\footnote{The attenuation code in DREAM makes heavily
    ...
    this lib.} %Also, the \texttt{fftconv\_p} and \texttt{sum\_fftconv\_p} functions needs the FFTW3 library.}
...
currently available for Linux (i686 [32-bit], x86\_64 [64-bit]), Windows (i686 [32-bit], x86\_64 [64-bit]), and for
Intel Macs (Mac OS X). The binaries are compiled using generic compiler flags and without support for the FFTW
library (for the transducer functions) and should, therefore, run on most setups.

If you want higher performance then it is recommended that you compile DREAM from source. From DREAM version $\ge$ 3.x
we have added support for the cross-platform \href{http://www.cmake.org/}{\emph{CMake}} build system (to replace the
bash-script and Unix Makefiles that was previously used).

There is
a bash script and m-files included in the source distribution to facilitate building the toolbox and,
furthermore, the \texttt{Makefile}s used in build process can easily be edited for further fine tuning of the
performance.

\subsection{Binary Installation (Matlab only)}

\begin{enumerate}

  \item Download the \texttt{dream-xxx-{\version}.tgz} file, or \texttt{dream-xxx-{\version}.zip} file, using your favorite browser
    (where \texttt{xxx} replaced by the hardware architecture (\texttt{unix} [32/64], \texttt{win} [32/64], or \texttt{maci}).

  \item Copy the file to a suitable directory on your disk and uncompress the file:
    \begin{description}
    \item[Windows:] Use \texttt{Winzip} or \texttt{Winrar}.
    \item[Linux:] Type \texttt{tar xvzf dream-xxx-{\version}.tgz}  (or \texttt{gunzip dream-xxx-{\version}.zip}).
    \end{description}

    \item Add the new directory into the \Matlab\ path. This can be performed by choosing the
      \emph{Set Path} command from the \textbf{File menu} (\Matlab\ only) or using the \texttt{addpath} command.
      On Linux you can add the line
      \texttt{addpath('/}\emph{your\_installation\_dir}\texttt{/)'}
      to your \texttt{startup.m} file.
\end{enumerate}

Note: since the oct-API often changes between releases of Octave the binaries will probably only work for
the version of Octave that they were compiled for. For this reason binaries for Octave are no longer
available and you need to install the toolbox from source which easily can be done with the Octave
\texttt{pkg} command (see Section~\ref{sec:oct-install}).

\subsection{Windows Specific Installation Notes}
\label{sec:win-install}

If you want to use the parallel (threaded) DREAM functions you need to install the
Pthreads-Win32 library. Download the library from:
%
%\begin{verbatim}
     \url{http://sourceware.org/pthreads-win32/}
%\end{verbatim}
%
and put the \\ \href{ftp://sourceware.org/pub/pthreads-win32/dll-latest/lib/pthreadGC2.dll}{pthreadGC2.dll}
file in a suitable directory, such as, \texttt{C:{$\backslash$}WINDOWS{$\backslash$}System32}; this file is
now also available on the DREAM web page. If you want to build the Pthreads library for Windows from source
then you can find instructions in Appendix~\ref{sec:build_pthreads-windows}.


The \texttt{fftconv\_p} and \texttt{sum\_fftconv\_p} use the FFTW3 library (see Section~\ref{sec:misc-conv}).
...
%\end{verbatim}
%
but it is now also available on the DREAM web page.
%Rename the \texttt{libfftw3-3.dll} file to
%\texttt{fftw3.dll} and
Copy the \texttt{libfftw3-3.dll} file to \texttt{C:{$\backslash$}WINDOWS{$\backslash$}System32}.
If you want to build the FFTW library for Windows from source
then you can find instructions in Appendix~\ref{sec:build_fftw-windows}.

To install DREAM for Octave on Windows see Sections~\ref{sec:win_octave_mingw}.

\subsection{MacOS X Specific Installation Notes}
\label{sec:mac-install}

The \texttt{fftconv\_p} and \texttt{sum\_fftconv} functions use the FFTW3 library
(see Section~\ref{sec:misc-conv}). Instructions for installing FFTW on MacOS X can be found at:

\url{http://www.fftw.org/install/mac.html}

\noindent If Octave is installed using, for example MacPorts, then the FFTW library should already be installed.
...
To build the DREAM Toolbox from sources you need to have developer tools installed for your
system (compiler, linker, etc.).
%
Start with downloading (and uncompressing) the (full) source \texttt{DREAM-{\version}.tgz} file.
This file contains the source code, the documentation (the user manual), and the html code
for the web pages. The build process is based on Makefiles both for C/C++ code and for
building the (\LaTeX) user manual and html documentation.
%
Therefore, to build the documentation you need to have \TeX/\LaTeX\ installed and, furthermore,
to build the html documentation you also need the
\href{http://www.cse.ohio-state.edu/~gurari/TeX4ht/mn.html}{tex4ht} and
\href{http://www.andre-simon.de/}{highlight} tools.\footnote{The build of the
documentation has only been tested on Linux based systems.}

\subsubsection{Build DREAM for Linux/Unix}
\label{sec:linux-build}

There are three methods that can be used to build the DREAM Toolbox on unix from sources. The first, and simplest,
is to use the m-script \texttt{build\_mexfiles\_unix.m}, the second is to use the bash script
\texttt{build\_dream} and the third is to manually edit the build configuration file \texttt{Make.inc}
and then compile the sources.

\noindent\emph{Method 1:} Using the \texttt{build\_mexfiles\_unix.m} script.

This is the simplest method but there is no optimization for the used architecture and the attenuation code
is build without FFTW support (see Section~\ref{sec:fftw}). This will also only build the mex-files
(not the oct-files).

\begin{enumerate}

\item Install \href{http://www.fftw.org}{FFTW} (if it is not already installed).

\item Start \Matlab\ and select compiler (gcc is recommended) with \texttt{mex -setup} (if you have not already done so).
\item Remove the \texttt{-ansi} flag from your
\begin{verbatim}
~/.matlab/R20XXx/mexopts.sh
\end{verbatim}
  file (the DREAM sources contain C++ style comments and they will not compile with \texttt{-ansi}).

\item Add \texttt{-std=c++11} to the CXXFLAGS in your
\begin{verbatim}
~/.matlab/R20XXx/mexopts.sh
\end{verbatim}
  file (the DREAM sources uses C++11 threads).

\item Run the m-script: \texttt{build\_mexfiles\_unix}

\item Copy the files in the \texttt{gui} and \texttt{help\_m\_files} directories to your
  DREAM install directory.

\item Add your DREAM install directory to the \Matlab\ path. That is, add\\
  \texttt{addpath('/}\emph{your\_installation\_dir}\texttt{/')}to your \texttt{matlab/startup.m} file.

\end{enumerate}
%
%Note: if you want fftw support for all transducer functions you can add the \texttt{-DUSE\_FFTW} option to each
%mex call in \texttt{build\_mexfiles\_unix.m}.


\noindent\emph{Method 2:} Build using CMake.
%
\begin{enumerate}

\item Install CMake (if not already done).

\item Create a build folder:\\
  \texttt{dream-src \$ mkdir build}\\
  \texttt{dream-src \$ cd build}

\item Configure the build:\\
  \texttt{build \$ cmake -DBUILD\_USERMAN=on -DCMAKE\_C\_FLAGS="-O2 -march=native" -DCMAKE\_CXX\_FLAGS="-O2 -march=native" -DCMAKE\_BUILD\_TYPE=Release ..}

\end{enumerate}

\noindent\emph{Method 3:} Using the \texttt{build\_dream} bash script. TODO: remove this!

The bash script \texttt{build\_dream} will probe the machine for cpu, architecture (32 or 64 bits)
and then use a pre-defined set of compiler flags for the machine (currently only x86 is supported).
This script will both build the mex-/oct-interfaces and the documentation for DREAM. It is assumed that \Matlab\
is installed in \texttt{/usr/local/matlab}. If you have installed \matlab\ somewhere else then you can create a symbolic
link to the \texttt{/usr/local/matlab} dir. The \texttt{build\_dream} script takes a few flags, use \texttt{-h} to
...
./build_dream [-native,-n|-auto,-a|-coppermine|-northwood|-prescott
        |-banias|-penryn|-nahelem|-westmere]

 builds the DREAM mex-files, oct-files, and documentation
 for a cpu architecture given by the 1st input argument.

 ./build_dream -native will build with -march=native and
 ./build_dream -auto will try to auto-detect the cpu arch.
 The other flags uses a pre-defined set of flags for the
 corresponding arch.

 If the second argument is -a or -affinity then DREAM will be
 build with CPU affinity control support.

\end{verbatim}

The \texttt{-native/-n} flag will build all sources with the (ggc) flags \texttt{-02 -march=native}, that is,
it lets \texttt{gcc} probe/detect the hardware and builds all DREAM sources optimized for the current hardware.
%
When \texttt{-auto/-a} flag is used the build script uses DREAM's own (somewhat outdated) detection code to select compiler flags.
One can also use pre-defined sets of flags for a few architecture, which currently are: \texttt{-coppermine},  \texttt{-northwood},
\texttt{-prescott}, \texttt{-banias}, \texttt{-penryn}, \texttt{-nahelem}, and  \texttt{-westmere}, repectively.

One can also use a second flag \texttt{-affinity/-a} which builds DREAM with experimental affinity control support. This is only
available for a few functions and has only been tested on Linux. When enabled one can select which cpu/core a thread should run on.

After the \texttt{build\_dream} script is done building the code and documantation add your DREAM install directory to
the \Matlab/Octave path(s). That is, add \texttt{addpath('/}\emph{your\_installation\_dir}\texttt{/')} to your
\texttt{matlab/startup.m} file and/or \texttt{.octaverc} file.

\noindent\emph{Method 3:} Manual configuration.
%
\begin{enumerate}

\item Copy \texttt{Make.default} to \texttt{Make.inc}, and open the \texttt{Make.inc} file with a
  text-editor.

\item Change the paths for Matlab and/or Octave in \texttt{Make.inc} to fit your installation.

\item Change \texttt{INSTALL\_DIR}, \texttt{MEX\_EXT} (if you use \Matlab),
  \texttt{CFLAGS}, and \texttt{OCTVER} (if you use Octave) in \texttt{Make.inc} to fit your architecture.

\item If you don't have both Matlab and Octave then open the \texttt{Makefile} and remove the corresponding software on
  the line ``\texttt{all: matlab octave doc}'' that you don't use.

\item Type \texttt{make}

\item Add your DREAM install dir to the \Matlab/Octave path(s). That is, add\\
  \texttt{addpath('/}\emph{your\_installation\_dir}\texttt{/')} to your \texttt{matlab/startup.m} file
  and/or \texttt{.octaverc} file.
\end{enumerate}


\subsubsection{Build the DREAM mex-files for MacOS X (Intel Macs)}

There is currently only one method to build the DREAM mex-files for MacOS X and that is to use
the \texttt{build\_mexfiles\_unix.m} script (the \texttt{Makefile}s do not currently
work under Mac OS X).

\begin{enumerate}

\item Install \href{http://www.fftw.org/install/mac.html}{FFTW} (if it is not already installed).
...
~/.matlab/R20XXx/mexopts.sh
\end{verbatim}
  file (the DREAM sources contain C++ style comments which will not compile with \texttt{-ansi}).
...
\item Add your DREAM install directory to your \Matlab\ path. That is, add \\
  \texttt{addpath('/}\emph{your\_installation\_dir} to your \texttt{matlab/startup.m} file
  (or use the menu in the \Matlab\ GUI).

\end{enumerate}
%
%Note: if you want fftw support for all transducer functions you can add the \texttt{-DUSE\_FFTW} option to each
%mex call in \texttt{build\_mexfiles\_unix.m}. Need to att -lfftw3 too.

\subsubsection{Build the DREAM Matlab 32-bit \texttt{mex}-files for Windows}

\noindent\emph{Method 1:} Using the MinGW/Gnumex Tools.

%MinGW is a iso C99 compiler which

\begin{enumerate}

\item \href{http://www.signal.uu.se/Toolbox/dream/download.shtml}{Download} the DREAM Toolbox full
  source package and uncompress it.

\item Download the MinGW-w64 compiler for your architecture (i686 [32-bit] or x86\_64 [64-bit]), see \url{http://mingw-w64.sourceforge.net/}).

\item Uncompress the corresponding zip-file, create the folder \texttt{C:\\MinGW-w32}, and copy all files to that folder.

\item Get the \texttt{phreadGC2.dll} and  \texttt{libfftw3-3.dll} (for your corresponding architecture) from the DREAM web-page.

\item Copy the \texttt{phreadGC2.dll} and  \texttt{libfftw3-3.dll} to the \texttt{windows/dll}/\texttt{windows/dll\_64}
  (depending on your architecture) folder.

\item Also, copy the headerfiles: \texttt{phread.h}, \texttt{shed.h}, and \texttt{semaphore.h}, and  \texttt{fftw3.h}, to the
  \texttt{windows/dll}/\texttt{windows/dll\_64}

\item Start Matlab and cd to the \texttt{windows} folder.
...
\end{verbatim}
  or
\begin{verbatim}
   >> build_mexfiles_mingw_64
\end{verbatim}
 This will build all mex-files and copy them, some help files, the GUI files, and the pthread and FFTW libraries,
 to an installation folder which is printed when the script is done.
...
\subsubsection{Build the DREAM Octave 32-bit \texttt{oct}-files for Windows using the MinGW Compiler}
\label{sec:win_octave_mingw}

The pkg source can be build also on Windows with resent versions of Octave (tested with Octave 3.2.3
[MinGW 4.4.0]). To do this, using the native Windows Octave from \href{http://octave.sourceforge.net}{octave-forge},
you only need to install Octave since FFTW, the MinGW compiler, and  the pthread libs
are included in the Octave package).
%
\begin{enumerate}

\item \href{http://www.signal.uu.se/Toolbox/dream/download.shtml}{Download}
  the DREAM Toolbox \texttt{pkg} source package (i.e., the file \texttt{dream-{\version}.tar.gz}).

\item  Download the Windows \href{http://octave.sourceforge.net}{Octave} installer.

\item Install Octave in \texttt{C:$\backslash$Octave}; do not install in
  \texttt{C:$\backslash$Program Files$\backslash$Octave} since the
  \texttt{mkoctfile} script that is used to compile the C++ code in the DREAM toolbox do work well
  with white spaces in the path (the default install location should be OK).

  % \item Get the \texttt{fftw3.h} file (for version 3.1.3) from \url{http://www.fftw.org}, or from
  %   \href{http://www.signal.uu.se/Toolbox/dream/download.shtml}{DREAM web page}, and copy it to
  %   \texttt{C:$\backslash$Octave$\backslash$include$\backslash$}; you don't need to install the fftw3 \texttt{dll}
  %   file since it is already included in the Windows package of Octave.

  % \item Get the \texttt{phread.h}, \texttt{shed.h}, and \texttt{semaphore.h} files from
  %   \url{http://sourceware.org/pthreads-win32/} (version 2.8.0), or from
  %   the \href{http://www.signal.uu.se/Toolbox/dream/download.shtml}{DREAM web page}, and copy them to
  %   \texttt{C:$\backslash$Octave$\backslash$include$\backslash$}

  % \item Get the \texttt{phreadGC2.lib} file from \url{http://sourceware.org/pthreads-win32/},
  %   or from the\\ \href{http://www.signal.uu.se/Toolbox/dream/download.shtml}{DREAM web page},
  %   copy it to \texttt{C:$\backslash$Octave$\backslash$lib$\backslash$}, and
  %   rename it to \texttt{phread.lib}; the Unix style Makefile which is used to build the toolbox
  %   expects this name of the lib file.

\item Build and install the DREAM Toolbox with: \texttt{pkg install dream-{\version}.tar.gz}.

  % \item Get the \texttt{pthreadGC2.dll} file (for gcc) from
  %   \url{http://sourceware.org/pthreads-win32/} (version 2.8.0), or from
  %   the \href{http://www.signal.uu.se/Toolbox/dream/download.shtml}{DREAM web page}, and copy
  %   it to \texttt{C:{$\backslash$}WINDOWS{$\backslash$}System32}.

\end{enumerate}

\subsubsection{Build the DREAM Octave \texttt{oct}-files for 64-bit Windows}
\label{sec:oct_win_64}

Currently there is no support for building 64-bit \texttt{oct}-files since there is no 64-bit version of
Octave for Windows available (yet).

\subsubsection{Compile with FFTW support for the Attenuation Code}
\label{sec:fftw}

The attenuation code in the DREAM Toolbox uses FFTs extensively. To speed up the FFT computations
(by 10\% to 50\%) one can compile The DREAM Toolbox with FFTW support for the attenuation code. To
do this you need to compile all functions with the flag \texttt{-DUSE\_FFTW} and link with
\texttt{-lfftw3}. This can be accomplished by changing:
%
\begin{verbatim}
ATT_FFTW       =
FFW_LIB        =
...
\end{verbatim}
%
in the file \texttt{Make.default} before compiling the toolbox if you are using \emph{Method 3} described in
Section~\ref{sec:linux-build} (the \texttt{build\_dream} uses FFTW by default).
...
points of interest, the so-called observation points. The observation points are located on a line
...
\begin{table}[tphp]
  ...
  \begin{tabular}{||l|l||}
    ...
    Spherical concave transducer (focused) & \texttt{dreamsphere\_f}\\
    Spherical convex transducer (defocused) & \texttt{dreamsphere\_d}\\
    Cylindrical concave transducer (focused) & \texttt{dreamcylind\_f}\\
    Cylindrical convex transducer (defocused) & \texttt{dreamcylind\_d}\\
    \hline
    Array with rectangular elements & \texttt{dream\_arr\_rect}\\
    Array with circular elements & \texttt{dream\_arr\_circ}\\
    Array with cylindrical concave elements (focused) & \texttt{dream\_arr\_cylind\_f}\\
    Array with cylindrical convex elements (defocused) & \texttt{dream\_arr\_cylind\_d}\\
...
\end{table}
%
By convention, a transducer function name ending with ``\texttt{\_f}'' is a focused transducer (or has
focused transducer elements) and a transducer function name ending with ``\texttt{\_d}'' is defocused
(convex) transducer.
...
divide the observation points in sets and let a different process handle each set.
On a multiprocessor machine, where each process (or thread) can run on a separate CPU core simultaneously
this will speed up the computations. In the DREAM toolbox this is performed using threads and
the SIRs for each observation point set is computed in its own thread.

In DREAM version $<3$ Pthread was used and the DREAM functions had an extra parameter, \texttt{n\_cpus}, that was
used to set the number of threads (all functions with thread support had a ``\texttt{\_p}'' in its function name). In
DREAM version $\ge 3$ C++11 threads is used instead and the number of CPU cores is auto-detected to avoid the extra
\texttt{n\_cpus} parameter. This reduces the number of DREAM functions and we don't need to build the Pthread lib
on, for example, Windows platforms. One must however build DREAM with a C++ compiler that supports the C++11
standard.\footnote{The \texttt{conv\_p} and \texttt{fftconv\_p} functions still uses the \texttt{\_p}
naming scheme to avoid a conflict with the build-in \texttt{conv} (Octave, Matlab) and \texttt{fftconv} (Octave) functions.}

To see if multiple threads are running using Linux or MacOS X, first run a DREAM function on one terminal
%
\begin{verbatim}
    >> H = dreamrect_p(Ro,geom_par,s_par,delay,m_par,'stop');
\end{verbatim}
%
and, second, open new terminal window and use the command \texttt{htop}.\footnote{On MacOS X \texttt{htop} can, for example, be installed
using MacPorts.} One should see something like:
%
\footnotesize
\begin{verbatim}
...
26799 fl         20   0  798M 67020 35492 S 386.  0.8  9:14.40 /usr/bin/octave-cli-3.8.2
26970 fl         20   0  798M 67020 35492 R 98.6  0.8  0:21.60 /usr/bin/octave-cli-3.8.2
26969 fl         20   0  798M 67020 35492 R 97.2  0.8  0:21.32 /usr/bin/octave-cli-3.8.2
26967 fl         20   0  798M 67020 35492 R 96.7  0.8  0:22.08 /usr/bin/octave-cli-3.8.2
26968 fl         20   0  798M 67020 35492 R 94.8  0.8  0:21.58 /usr/bin/octave-cli-3.8.2
...
The \texttt{conv\_p} and \texttt{fftconv\_p} functions computes the
one dimensional convolution of the columns in a matrix \texttt{A} and
...
a slow \texttt{for}-loop using the \texttt{conv} function
...
vectors. To use the \texttt{fftconv\_p} the FFTW3 lib must be
available. The windows version of \texttt{fftconv\_p} is linked to
the FFTW lib which, as previously described, can be found at
\texttt{ftp://ftp.fftw.org/pub/fftw/fftw3win32mingw.zip}.
...
both \texttt{conv\_p},  \texttt{fftconv\_p} and  \texttt{sum\_fftconv\_p} (see Section~\ref{sec:sumfftconv_p}) have an in-place
...
a copy of a matrix unless it is changed after \texttt{X = Y} assignment.

\subsubsection{Computing Array Responses for Arbitrary Input Signals}
\label{sec:sumfftconv_p}
...
%
The DREAM Toolbox has a function, \texttt{sum\_fftconv\_p}, to facilitate computation of
discrete array responses with arbitrary input signals. Similar to the \texttt{fftconv\_p} function,
the \texttt{sum\_fftconv\_p} function uses a FFT based algorithm to compute the convolutions.
The \texttt{sum\_fftconv\_p} function performs an operation similar to the code:
%%
\begin{verbatim}
  YF = zeros(M+K-1,N);
  for l=1:L
...
\end{verbatim}
%%
where \texttt{A} is a 3D matrix.

\begin{verbatim}
  [Y] = sum_fftconv_p(A,B);
  [Y] = sum_fftconv_p(A,B,wisdom_str);
        sum_fftconv_p(A,B,Y);
        sum_fftconv_p(A,B,Y,wisdom_str);
\end{verbatim}
%
\noindent Input parameters:
\begin{itemize}

\item [] \texttt{A} An $M\times N\times L$ three-dimensional matrix.

\item [] \texttt{B} A $K\times L$ matrix.
...
\section{Building the Pthreads Library for 32 and 64 bit Windows}
\label{sec:build_pthreads-windows}

The DREAM toolbox uses pthreads (POSIX threads) to run computations in parallel on several cores/cpus. To run the
parallel functions on Windows the \emph{Pthreads-win32} library is used:\\ \url{http://sourceware.org/pthreads-win32/}.
The most current version of the Pthreads-win32 library, which is the one used by DREAM, is version 2.8.0 (2006-12-22).
A 32-bit binary version of the library can be found at: \url{ftp://sourceware.org/pub/pthreads-win32/}.
%
The \href{http://sourceforge.net/projects/mingw-w64/}{MinGW-w64} project has a patch to build the Pthreads-win32 lib
for 64-bit Windows, and there are some more 64-bit patches at:\\ \url{http://www.cadforte.com/wiki/index.php/Pthreads}.

To facilite building the lib we have included our own (experimental) patch, which is a combination of the patches
above (with some minor changes), and the source code for the Pthreads-win32 lib in the source code package of the DREAM
toolbox. There are also two (bash) build scripts which can be used for building the 32-bit and 64-bit libs,
respectively. This is tested using the MinGW-w64 cross compiler tool chains on Linux. After uncompressing
the DREAM source code packge you can find these files in the \texttt{windows} folder.

Given that you have installed the MinGW-w64 i686 cross compiler you can
build the 32-bit Pthreads-win32 with
%
\begin{verbatim}
# ./build_pthreads_win32
\end{verbatim}
%
This will (patch it for the MinGW-w64 compiler) build the \texttt{pthreadGC2.dll} file and copy it (the \texttt{pthread.def} file,
and the header files \texttt{pthread.h}, \texttt{sched.h} and, \texttt{semaphore.h}) to  the \texttt{windows/dll} folder.

For 64-bit windows you first need to install the MinGW-w64 x86\_64 cross compiler and then the script
\begin{verbatim}
# ./build_pthreads_win64
\end{verbatim}
%
will copy the Pthreads-win32 sources, patch them for 64-bits support, and build the lib. The 64-bit lib (the
\texttt{pthread.def} file, and the header files \texttt{pthread.h}, \texttt{sched.h} and,
\texttt{semaphore.h}) will be installed in the  \texttt{windows/dll\_64} folder.

%Note: if you intend to use these libs with one of the MSVC compilers then you also need the
%corresponding \texttt{lib} files. These \texttt{lib} files can be generated using the windows
%bat-files found in the \texttt{windows/dll} and  \texttt{windows/dll\_64} folders, respectively.


\section{Building the FFTW Library for 32 and 64 bit Windows}
\label{sec:build_fftw-windows}

For conveniance we have also included scripts to build the FFTW library
for both 32 and 64-bit Windows. The (bash) scripts uses MinGW-w64 (i686 and x86\_64) cross compiler
tool chains, respectively. Change directory to the \texttt{windows} folder and run
%
\begin{verbatim}
# ./BUILD-MINGW-FFTW
\end{verbatim}
...
\end{verbatim}
%
for 64-bit Windows. The two build scripts are based on the corresponding ones at the FFTW
\href{http://www.fftw.org}{site} adapted for building DREAM on Windows. The scripts downloads the FFTW
sources, builds and install the libraries in the \texttt{windows/dll} and  \texttt{windows/dll\_64} folders,
respectively.

%Similary to the Pthreads-win32 lib you need to build the \texttt{lib} files if you intend to use
%these libs with one of the MSVC compilers. These \texttt{lib} files can be generated using the corresponding
%windows bat-files found in the \texttt{windows/dll} and  \texttt{windows/dll\_64} folders, respectively.
