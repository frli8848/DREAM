...
\subsection{Delay-and-sum Imaging  --- SAFT and TFM}
\label{sec:das}
...
(TFM).
%
Both methods are special cases of methods based on so-called \emph{delay-and-sum} imaging (DAS).
The idea is just to compensate for the double-path propagation delay from each transmitter/receiver to each
...
The DREAM Toolbox has a function \texttt{das} which can operate in both SAFT and TFM mode.
The SAFT mode (transmit with one element - receive with one element) is activated by supplying
an empty matrix for the receive element positions (the third arg to \texttt{das} is \texttt{[]}):
%
\begin{verbatim}
   Im = das(Y, G, [], Ro, dt, delay, cp);
\end{verbatim}
%
...
The difference between SAFT and TFM is that in SAFT one transmit with one element and receives
with one element whereas one in TFM transmits with all elements and receives with all elements.
The resulting dataset used in TFM is often called \emph{full matrix capture} (FMC) since one
have data from every receive element for every transmit element. The dataset is therefore an order
of magnitude larger than the dataset used in sAFT.

In TFM mode one needs to supply both transmit element and receive element grid matrices
(\texttt{Gt} and \texttt{Gr}) to \texttt{das}:
%
\begin{verbatim}
   Im = das(Y, Gt, Gr, Ro, dt, delay, cp);
\end{verbatim}
%
\noindent Input parameters in TFM mode:
\begin{verbatim}
