...
divide the observation points in sets and let a different process (thread) handle each set.
...
In DREAM, C++11 threads is used and number of CPU cores is auto-detected to avoid and extra
\texttt{n\_cpus} parameter which previously was used. One can, however, control the number of threads with
the \texttt{NUM\_DREAM\_THREADS} environment variable and on Linux and macOS one can do this using
\footnotesize
\begin{verbatim}
  $ export NUM_DREAM_THREADS=4
\end{verbatim}
\normalsize
%
for example.
