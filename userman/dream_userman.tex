...
\usepackage{fancyvrb} % Frames around verbatim block
...
\title{\textbf{User Manual for the DREAM Toolbox\\
  ...
  \href{https://www.python.org/}{\textsc{Python}},
  and
  \href{https://julialang.org}{\textsc{Julia}}}
...
for wide band (pulse) excitation both in near and far fields.
...
will use a circular transducers.
%for this example but more advanced examples can be found on the DREAM weba
%page  \url{http://www.signal.uu.se/Toolbox/dream/} on the
%\emph{Examples} page.
More details of the various functions in the DREAM Toolbox can be found in
...
at $z=10$ mm, from 1--50 mm with 1 mm between them (using m-code):
%
\begin{lstlisting}[language=Matlab,
  ...
  caption={Defining observation points.},
  label={code:obs-points}]
...
\begin{lstlisting}[language=Matlab,
  ...
  frame=single,
  commentstyle=\color{red},
  caption={Defining descretization parameters.},
  label={code:discr-pars}]
...
\begin{lstlisting}[language=Matlab,
  ...
  frame=single,
  commentstyle=\color{red},
  caption={Defining material parameters.},
  label={code:meterial-pars}]
% Material parameters.
...
\begin{lstlisting}[language=Matlab,
  ...
  frame=single,
  commentstyle=\color{red},
  caption={Defining geometrical parameters.},
  label={code:geom-pars}]
...
\begin{lstlisting}[language=Matlab,
  ...
  frame=single,
  commentstyle=\color{red},
  caption={Defining SIR starting point.},
  label={code:delay-pars}]
% Delay.
t_z = z*1e3/cp;
...
For Python, the corresponding code (with plotting added) is:
\begin{lstlisting}[language=Python,
  ...
  frame=single,
  commentstyle=\color{red},
  caption={A short Python \texttt{dreamcirc} example.},
  label={code:short-example-python}]
import math
import numpy as np
...
\end{lstlisting}
%
and the Julia code (which is very similar to the m-code) is:
%
\begin{lstlisting}[language=Python,
  basicstyle=\scriptsize,
  breaklines=true,
  ...
  frame=single,
  commentstyle=\color{red},
  caption={A short Julia \texttt{dreamcirc} example.},
  label={code:short-example-julia}]
using LinearAlgebra
using Plots
...
\PARstart{T}{he} transducer functions are implemented as \emph{mex}-functions for \Matlab,
\emph{oct}-functions for Octave, and as modules for Python; the bindings are pre-compiled code that is
dynamically linked to \Matlab/Octave/Python at run time. This greatly increases the computation speed
compared to using ordinary m-files or Python code since the DR-concept (see~\cite{Piwakowski1999}) uses
\texttt{for} and \texttt{while} loops extensively. An overview of the transducer functions can be found
...
When using Python one need to import the corresponding function before using it like, for example;
%
...
\end{Verbatim}
%
and for Julia one need to import the DREAM function using the \texttt{using} keyword
with an \texttt{\_m} appended after the function name likem for example:
%
\begin{Verbatim}[frame=single]
  using dreamcirc_m
...
\end{Verbatim}
%
For MATLAB and Octave it is enough to set the path to the DREAM Toolbox
(for more information see Section~\ref{sec:installation}).
...
\item \texttt{'ignore'}: Here an error is silently ignored (and err output arg is non-zero),

\item \texttt{'warn'}: A warning message is printed but computation is not stopped (and err output arg is non-zero),

\item \texttt{'stop'}: An error message is printed and the computation is stopped (and none of the output args is set).
...
The second (optional) output argument, \texttt{err}, is non-zero if an error has occurred
and 0 otherwise.
%
\begin{Verbatim}[frame=single]
  [H,err] = dream***( ... );
...
\noindent \textbf{Syntax} m-code (MATLAB and Octave):
\begin{Verbatim}[frame=single]
  H = dreamline(Ro,geom_par,s_par,delay,m_par,err_level);
\end{Verbatim}
%
\noindent \textbf{Syntax} Python:
\begin{Verbatim}[frame=single]
  import dreamline as dl

  H = dl.dreamline(Ro,geom_par,s_par,delay,m_par,err_level)
\end{Verbatim}
%
\noindent \textbf{Syntax} Julia:
\begin{Verbatim}[frame=single]
  using dreamline_m

  H = dreamline_m.dreamline(Ro,geom_par,s_par,delay,m_par,err_level);
\end{Verbatim}
...
For Python and Julia the syntax for calling the other DREAM functions is similar to calling the \texttt{dreamline}
function and will therefore not be repeated.

\subsubsection{Rectangular Transducer}

The size of the rectangular transducer is determined by \texttt{a} and \texttt{b}.

\noindent \textbf{Syntax} m-code:
\begin{Verbatim}[frame=single]
...
\noindent \textbf{Example m-code (linear array)}:
\footnotesize
\begin{Verbatim}[frame=single]
  gx = -10:1:10;
...
\noindent \textbf{Example Python/numpy code (linear array)}:
\footnotesize
\begin{Verbatim}[frame=single]
...
\begin{eqnarray}
  ...
  w _{\text{hann}}(r) & = & 0.5 - 0.5 \cos(r \pi/r_{\text{max}}) \\
  w _{\text{hann}}(r) & = & 25/46 - (1 - 25/46) \cos(r \pi/r_{\text{max}}) \\
...
\item[\texttt{'hann'}]   - Hann window
\item[\texttt{'hamming'}]   - Hamming window
...
\subsection{Delay-and-sum Synthetic Aperture Imaging --- SAFT, TFM and RCA}
\label{sec:das}

The DREAM Toolbox contains two delay-and-sum based synthetic apertue imaging (SAI) reconstruction
methods: the \emph{synthetic aperture focusing technique} (SAFT) and the \emph{total focusing method}
(TFM). The latter one has two modes: linear array TFM and one mode for so-called row-column
addressed (RCA) arrays.
%
These methods are all variants of \emph{delay-and-sum} imaging (DAS). The idea with DAS is just to
compensate for the double-path propagation delay from each transmitter/receiver to each
observation point and then perform a (coherent) summation. This operation is essentially based on an
geometrical optics approach and is analogous to the operation of an acoustical lens~\cite{Kino1987}.

SAFT is based on \emph{synthetic aperture imaging} (SAI) which was developed to improve
resolution in the along track direction for side-looking radar. The idea was to record
...
{
  \begin{equation}
    \text{Im}(x_{\tilde{n}},z_m) = \sum_{n} w_n y(x_n,\frac{2}{\cp}
...
%%
where $\text{Im}(x_{\tilde{n}},z_m)$ is the beamformed (or reconstructed) image.

The DREAM Toolbox has a function \texttt{das} which can operate in both SAFT and TFM/RCA mode.
%
\begin{verbatim}
   Im = das(Y, Gt, Gr, Ro, dt, delay, cp, method, err_level, device);
\end{verbatim}
%
The SAFT mode (transmit with one element - receive with one element) is activated by supplying
an empty matrix for the receive element positions (the third arg to \texttt{das} is \texttt{[]}) and
setting DAS mode string to \texttt{'saft'}:
%
\begin{verbatim}
   Im = das(Y, G, [], Ro, dt, delay, cp, 'saft');
\end{verbatim}
%
\noindent Input parameters in SAFT mode:\\
\begin{tabular}{ l  l }
  %\hline
  \texttt{Y}  &  A $K\times L$ Ultrasonic B-scan data matrix where $K$ is the A-scan length \\
              & and $L$ is the number of transducer positions.\\
  \texttt{G}     & An $L\times 3$ transducer position grid matrix $[x_1 y_1 z_2; x_2 y_2 z_2; \cdots x_L y_L z_L]$; \\
  \texttt{Ro}    & A matrix of the form  $[xo_1 yo_1 zo_2; xo_2 yo_2 zo_2; \cdots xo_N yo_N zo_N]$; \\
              & where $N$ is the number of obervation (reconstruction) points.\\
  \texttt{delay} & A scalar (or vector) of starting point(s) of the A-scans in \texttt{Y}. \\
  \texttt{cp}    & Sound speed [m/s]. \\
  %\hline
\end{tabular}

The difference between SAFT and TFM (or RCA) is that in SAFT one transmit with one element and receives
with one element (the same element) whereas one in TFM/RCA transmits with all elements and receives with all
elements. The resulting dataset used in TFM/RCA is often called \emph{full matrix capture} (FMC) since one
have data from every receive element for every transmit element. The dataset is therefore an order
of magnitude larger than the dataset used in SAFT (which reduces the noise in the resulting image).

The difference between TFM and RCA mode is that in RCA mode the elements are arranged in a crossed layout
to form a 2D array whereas one in TFM mode a linear array is assumed; the datasets have the same size
if we use the same number of transmit and receive elements in both methods.

In TFM/RCA mode one needs to supply both transmit element and receive element grid matrices
(\texttt{Gt} and \texttt{Gr}) to \texttt{das} and set the DAS mode string to \texttt{'tfm'}
or \texttt{'rca'}:
%
\begin{Verbatim}[frame=single]
   Im = das(Y, Gt, Gr, Ro, dt, delay, cp, 'tfm');
\end{Verbatim}
%
or
%
\begin{Verbatim}[frame=single]
   Im = das(Y, Gt, Gr, Ro, dt, delay, cp, 'rca');
\end{Verbatim}
%
\noindent Input parameters in TFM/RCA mode:\\
\begin{tabular}{ l  l }
  \texttt{Y}     & $K\times L_t L_r$ Ultrasonic FMC B-scan data matrix. \\
                 & where $K$ is the A-scan length and $L_t $ and $L_r$ are the number of transmit \\
                 & (or column) and recieve (or row) transducer positions. \\
  \texttt{Gt}    & An $L_t$ x 3 transmit (or column) element position grid matrix \\
                 & Gt = $[xt1 yt1 zt2; xt2 yt2 zt2; ... xtL_t ytL_t ztL_t]$; \\
                 & where $L_t$ is the number of transmit element positions. \\
  \texttt{Gr}    & An $L_r \times 3$ receive (or row) element position grid matrix \\
                 & Gr = $[xr_1 yr_1 zr_2; xr_2 yr_2 zr_2; ... xr_{L_r} yr_{L_r} zr_{L_r}]$; \\
                 & where $L_r$ is the number of receive element positions. \\
  \texttt{Ro}    & A matrix of the form  [$xo_1 yo_1 zo_2; xo_2 yo_2 zo_2; \cdots xo_N yo_N zo_N$]; \\
                 & where $N$ is the number of obervation (reconstruction) points. \\
  \texttt{delay} & A scalar (or vector) of starting point(s) of the A-scans in \texttt{Y}. \\
  \texttt{cp}    & Sound speed [m/s]. \\
...
For RCA arrays \texttt{Gt} is the center positions of the columns and \texttt{Gr} is the center position of the
rows, respectively.

NB. The \texttt{das} function outputs a vector, \texttt{Im}, which one needs to reshape if one, for example,
wants to present the results as an image.

If one have build DREAM with OpenCL (GPU) support then there is also a function \texttt{das\_uni} which assume \emph{uniform}
grids both for the array element positions and the observation (image) points. This reduces the amount of data
transfered and stored on the GPU since we do not transfer the \texttt{Ro}, \texttt{Gt}, and \texttt{Gr} matrices
to the GPU.
